...
class TCPTunnel(_Tunnel):
    ...
    def __init__(
        ...
        arg_count = (gateway_ip is not None) + (gateway_path is not None) + (connect_cb is not None)
        if arg_count > 1:
            raise XKNXException("Only one of gateway_ip, gateway_path and connect_cb may be set")
        elif arg_count == 0:
            raise XKNXException("One of gateway_ip, gateway_path and connect_cb must be set")
    ...
    def _init_transport(self) -> None:
        ...
        if self.connect_cb is not None:
            ...
            self.transport = TCPTransport(
                remote_addr=('0.0.0.0', 0),
                connection_lost_cb=self._tunnel_lost,
                connect_cb=lambda loop, protocol_factory: loop.create_unix_connection(protocol_factory, path=self.gateway_path),
            )
        else:
            self.transport = TCPTransport(
                remote_addr=(self.gateway_ip, self.gateway_port),
